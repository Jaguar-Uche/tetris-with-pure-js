...
export let T2 = {
  ...
  draw: function (x, y) {
    (currentShape[0] = T2), drawBlock(x, y);
    drawBlock(x, y + 25);
    drawBlock(x - 25, y + 25);
    drawBlock(x, y + 50);
  },
    ...
    if (game.running) {
      ...
      this.timeoutId = setTimeout(() => {
        if (this.downMoveable) {
          ...
          occupy();
          dropAndClear();
          checkGameOver(this, game.indexC, game.indexD);
  ...
  updateRotability: function () {
    this.rotable1 =
      checkEmpty(game.indexB + 1) && checkSameRow(game.indexB, game.indexB + 1);
    this.rotable2 =
      checkEmpty(game.indexB + 1) && checkSameRow(game.indexB, game.indexB + 1);
  },
};
